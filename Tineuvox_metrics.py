...
parser.add_argument('--dir', type = str, default = None , help ='images path')
args = parser.parse_args()

data_path = os.path.join(args.dir,'test','ours_70000')
psnr_cal = 0
ssim_cal = 0
lpips_cal = 0
...
for idx in tqdm(range(len(gt))):
    ...
    estim1 = torch.Tensor(estim[idx][None,...]).cuda()
    gt1 = torch.Tensor(gt[idx][None,...]).cuda()

    errors = estim_error(estim1, gt1)
    psnr_list.append(errors["psnr"])
    ssim_list.append(errors["ssim"])
    lpips_list.append(errors["lpips"])
    # print(str , errors)

scores_dict = {}
scores_dict['PSNR'] = mean(psnr_list)
scores_dict['SSIM'] = mean(ssim_list)
scores_dict['LPIPS'] = mean(lpips_list)

print('PSNR: {}'.format(scores_dict['PSNR']))
print('SSIM: {}'.format(scores_dict['SSIM']))
print('LPIPS: {}'.format(scores_dict['LPIPS']))

with open(args.dir + "/scores_results.json", 'w') as fp:
